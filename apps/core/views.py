...
def mark_as_read(request, message_id):
    # FIX: removed pointless try/except, get_object_or_404 raises Http404 not DoesNotExist
    message = get_object_or_404(ContactMessage, id=message_id)
...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():          # FIX 1: was form.is_valid (missing parentheses)
            form.save()
            return redirect('auth:login')
    else:
        form = SignUpForm()           # FIX 2: was SignUpForm(request.POST) on GET request
